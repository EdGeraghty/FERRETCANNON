...
fun Route.federationV1Membership() {
    ...
    put("/invite/{roomId}/{eventId}") {
        // v1 invite is deprecated - redirect to v2
        println("Federation v1 invite endpoint called - returning 404 (use v2 instead)")
        call.respond(HttpStatusCode.NotFound, buildJsonObject {
            put("errcode", "M_UNRECOGNIZED")
            put("error", "v1 invite endpoint is deprecated. Please use /_matrix/federation/v2/invite/ instead.")
        })
    }
