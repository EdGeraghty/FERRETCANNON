...
fun Route.roomRoutes() {
    ...
    // GET /rooms/{roomId}/joined_members - Get joined members (simplified format)
        ...
        try {
            ...
            if (roomId == null) {
                call.respond(HttpStatusCode.BadRequest, buildJsonObject {
                    ...
                    put("error", "Missing roomId parameter")
                })
                return@get
            }
            ...
            if (currentMembership != "join") {
                call.respond(HttpStatusCode.Forbidden, buildJsonObject {
                    ...
                    put("error", "User is not joined to this room")
                })
                return@get
            }

            // Get all joined members with their display names and avatar URLs
            val joinedMembers = transaction {
                Events.select {
                    (Events.roomId eq roomId) and
                    ...
                    if (membership == "join") {
                        val displayName = content["displayname"]?.jsonPrimitive?.content
                        val avatarUrl = content["avatar_url"]?.jsonPrimitive?.content
                        
                        memberUserId to buildJsonObject {
                            if (displayName != null) {
            ...
            call.respond(buildJsonObject {
                put("joined", JsonObject(joinedMembers))
            })
